...
# Install jq, dos2unix, GCP CLI based on https://cloud.google.com/sdk/docs/
ENV GPC_RPM_URL https://packages.cloud.google.com/yum/repos/cloud-sdk-el8-x86_64
RUN yum install -y dnf-utils jq dos2unix && \
...
# Freshen up base image and install ansible, kubectl, man, vim pages
RUN yum -y update && \
    yum install -y git zip epel-release python3-pip dnf-utils tmux wget man vim && \
